...
### NON-interactive
w1 <- ggplot(data=water, aes(x = Global_avg_footprint, y = protein, color=kingdom)) +   
...
w1 <- w1 + geom_text(aes(label=ifelse(
                                ...
                                Products,'')), hjust=0,just=0, size=3)
w1 <- w1 + scale_color_manual(values=c("darkred", "green4")) + 
    geom_smooth(data = water, method="lm", fullrange=TRUE)
w1 <- w1 +  xlab("Water Footprint: Global Average ($m^3/ton$") +
    ylab("Protein (%)") +
    ggtitle("Water Footprint vs. Protein of Raw Foods") + 
...
pairs(USJudgeRatings[,1:4], lower.panel = panel.smooth, upper.panel = panel.cor)

pairs(water[,c(3:7),with=F], lower.panel = panel.smooth, upper.panel = panel.cor,
      main = "water footprint data", 
      col = 2 + (as.numeric(water$kingdom) < 2)) ## color: red=2,green=3, plant=1,animal=2
