...
svg("Gl_adjfp_prot_low.svg")
grid.table(water[1:10,.( Products, L_per_g_Protein=round(G_L_per_g_protein) )])
dev.off()

svg("Gl_adjfp_prot_10.svg")
grid.table(water[protein>10,.( Products, L_per_g_Protein=round(G_L_per_g_protein) )])
dev.off()
```
![Global protein adjusted footprint](./Gl_adjfp_prot_low.svg "10 Least Water Intensive Food (Globally)" )

![Global protein adjusted footprint](./Gl_adjfp_prot_10.svg "Protein Adjusted Water Footprint of Foods > 10 g Protein (Globally)" )



```{r table_protein_CA, echo=F, eval=T}
require(gridExtra)
setkey(water, CA_L_per_g_protein)
svg("CA_adjfp_prot_low.svg")
grid.table(head( na.omit(water[,.( Products, L_per_g_Protein=round(CA_L_per_g_protein) )]),10 ))
dev.off()

svg("CA_adjfp_prot_10.svg")
grid.table(head( na.omit(water[,.( Products, L_per_g_Protein=round(CA_L_per_g_protein) )]),10 ))
grid.table(na.omit( water[protein>10,.( Products, L_per_g_Protein=round(CA_L_per_g_protein) )] ))
dev.off()
```

![Global protein adjusted footprint](./CA_adjfp_prot_low.svg "10 Least Water Intensive Food (in California)" )

![Global protein adjusted footprint](./CA_adjfp_prot_10.svg "Protein Adjusted Water Footprint of Foods > 10 g Protein (in California)" )
