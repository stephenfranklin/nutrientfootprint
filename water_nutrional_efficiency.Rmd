---
title: "The Healthiest Water-Efficient Foods."
...
par(mfrow = c(1,1))
pairs.panels(water[,c(3:7),with=F], smooth=T, ellipses=F, cor=T,
...
# Protein Adjusted Water Footprints

```{r table_protein_global, echo=F, eval=T}
require(gridExtra)
setkey(water, G_L_per_g_protein)
Gl_adjfp_prot_low <- xtable(water[1:10,.( Products, L_per_g_Protein=round(G_L_per_g_protein) )],align=c('c','c','c'))
kable(Gl_adjfp_prot_low, format = "html", caption = "10 Least Water Intensive Foods (Globally)")

Gl_adjfp_prot_10 <- xtable(water[protein>10,.( Products, L_per_g_Protein=round(G_L_per_g_protein) )], align=c('c','c','c'))
...
CA_adjfp_prot_low <- xtable(head( na.omit(water[,.( Products, L_per_g_Protein=round(CA_L_per_g_protein) )]),10 ),align=c('c','c','c'))
kable(CA_adjfp_prot_low, format = "html", caption = "10 Least Water Intensive Foods (in California)")

CA_adjfp_prot_10 <- xtable(na.omit( water[protein>10,.( Products, L_per_g_Protein=round(CA_L_per_g_protein) )] ),align=c('c','c','c'))
...
qplot(differences, aes(Products, California_footprint, Global_avg_footprint)) + geom_bar(position="dodge")




b <- ggplot(differences, aes(y=difference, x=Products, fill=))
b + geom_bar(stat="identity", position="dodge") +
    labs(x = "") +
    labs(y = "Relative annual change (percentage)") +
    labs(title = expression(PM[2.5] * " On-road Emissions: City Comparison")) +
    geom_hline()
...
ggplot(data = differences) +
    aes(Products, fill = difference) + 
    geom_bar(data = differences, stat = "identity") + 
