...
For the animal sourced foods, the studies also separated out four categories of farming systems: grazing, industrial, a mix of those two, and a weighted average for each nation. We're only looking at the weighted average of the U.S. (The studies don't break it down by State, as they do for plant sourced foods.)
...
CA_adjfp_prot_low <- xtable(head( na.omit(water[,.( Products, "Liters per Gram Protein"=round(CA_L_per_g_protein), "% protein"=round(protein) )]),10 ),align=c('c','c','c','c'))
...
Gl_adjfp_prot_low <- xtable(water[1:10,.( Products, "Liters per Gram Protein"=round(G_L_per_g_protein), "% protein"=round(protein) )],align=c('c','c','c','c'))
...
A few high protein foods already stand out as being unusually water efficient. In the California list, three animal sourced foods: turkey, chicken, and eggs,  and in the global list, Brazil nuts and oats. All five are interesting exceptions. I'll talk more about Brazil nuts further on. The percent of protein in oats should probably be reduced by half because it needs twice its mass in water to be edible, but still, getting 8 $\frac{1}{2}$ grams of protein by eating 100 grams of oatmeal that consumed 120 liters of water to grow isn't bad. Turkey, chicken, and eggs are immediate winners as far as water efficiency per protein, but because we already consume so much of them, they actually represent a huge amount of agricultural water.

The most water efficient proteins on the lists are from garlic, spinach, and brussel sprouts, and these have low percentages of protein, so you would have to eat a lot of them, and you could, but most people don't.  

So let's filter out the low protein foods:
...
require(xtable)
setkey(water, CA_L_per_g_protein)
CA_adjfp_prot_10 <- xtable(head(na.omit( water[protein>10,.( Products, "Liters per Gram Protein"=round(CA_L_per_g_protein), "% protein"=round(protein) )] ), 10),align=c('c','c','c','c'))
...
require(xtable)
setkey(water, G_L_per_g_protein)
Gl_adjfp_prot_10 <- xtable(head(water[protein>10,.( Products, "Liters per Gram Protein"=round(G_L_per_g_protein), "% protein"=round(protein) )],10), align=c('c','c','c','c'))
...
dairy p7
eggs 5.05 billion p8
turkey 465 million lbs 
