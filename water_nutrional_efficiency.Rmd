...
w1 <- w1 + geom_text(aes(label=ifelse(
                     ...
                     show_guide = F)
w1 <- w1 + labs(y=expression("Water Footprint: Global Average ("*m^3/ton*")"), x = "Protein (%)", title="Water Footprint vs. Protein of Raw Foods")
w1 <- w1 + theme(axis.title=element_text(face="bold.italic", 
   color="brown"), legend.position="top")
w1 + expand_limits(y = c(0, 17500), x = c(0, 30) )
```

# Footprint vs. Protein (Interactive)
...
w2$chart(showControls = T)  ## turn on magnify.
w2
#w2$show('iframe', cdn = TRUE)
